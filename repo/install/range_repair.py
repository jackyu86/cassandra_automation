...
def main():
    ...
    parser.add_option("-N", "--nodeip", dest="nodeip", default="",
                  metavar="NODEIP", help="node ip")
    ...
    subprocess.call(["./cqlsh","172.19.153.48","-e","update cfg.repair_job_info set last_start_time=toTimestamp(now()), dc='"+options.datacenter+"' where ip='"+options.nodeip+"' and ks='"+options.keyspace+"'"]);
    repair(options)
    subprocess.call(["./cqlsh","172.19.153.48","-e","update cfg.repair_job_info set last_end_time=toTimestamp(now()) where ip='"+options.nodeip+"' and ks='"+options.keyspace+"'"]);
    exit(0)
